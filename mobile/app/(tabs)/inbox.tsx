import { View, Text, FlatList, ActivityIndicator, TouchableOpacity, Alert, ScrollView, Modal, Keyboard, TextInput } from 'react-native';
...
export default function Inbox() {
  ...
  useEffect(() => {
    ...
    async function fetchInbox() {
      try {
        ...
        const data = await getInboxMeals(householdId);
        setSuggestions(data);
      } catch (error) {
        console.error(error);
      } finally {
  ...
  const handleAddSuggestion = useCallback(async () => {
    ...
    if (!inputValue.trim() || !u || !householdId) return;
    setAdding(true);
    try {
      await addSuggestion(
        inputValue,
  ...
  const handleReject = useCallback(async () => {
    ...
    try {
      await rejectSuggestion(rejectTarget);
      setSuggestions(prev => prev.filter(s => s.id !== rejectTarget));
  ...
  const renderItem = useCallback(({ item, index }: { item: Suggestion, index: number }) => {
    ...
    return (
      <View>
        ...
        <GlassCard className="p-0 border-0 shadow-none bg-transparent">
          <View className="flex-row justify-between items-start mb-4">
            <View className="flex-1 mr-4">
              ...
              <View className="flex-row items-center mt-1.5">
                ...
                {item.forDate && (
                  <View className="flex-row items-center bg-indigo-50/80 px-2 py-0.5 rounded-lg ml-3 border border-indigo-100/50">
                    <Calendar size={10} color="#6366F1" />
                    <Text className="text-indigo-700 text-[10px] font-black uppercase tracking-widest ml-1">
            ...
            <View className={`w-12 h-12 rounded-2xl flex items-center justify-center border ${isApproved ? 'bg-emerald-500/20 border-emerald-500/30' : 'bg-white/60 border-white/40 shadow-sm'}`}>
          ...
          <View className="flex-row justify-between items-center mt-2 border-t border-gray-200 pt-4">
            {isApproved ? (
              <View className="flex-row items-center justify-between w-full">
                <View className="bg-emerald-500/90 px-4 py-2 rounded-xl flex-row items-center shadow-lg shadow-emerald-200/50">
                  <Check size={14} color="white" strokeWidth={3} />
                  <Text className="text-white text-[10px] font-black uppercase tracking-widest ml-2">Godkjent</Text>
                </View>
                <GlassButton
                  variant="ghost"
                  icon={<Trash size={18} color="#EF4444" opacity={0.5} />}
              ...
              <View className="flex-row items-center justify-between w-full">
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                  className={`flex-row items-center px-5 py-2.5 rounded-[20px] shadow-sm ${hasVoted
                    ? 'bg-indigo-600/90 shadow-indigo-300/40'
                    : 'bg-white/50 border border-white/60'
                    }`}
                >
                  <ThumbsUp
                    size={16}
                    color={hasVoted ? '#FFFFFF' : '#9CA3AF'}
                    fill={hasVoted ? '#FFFFFF' : 'transparent'}
                  />
                  <Text className={`ml-2.5 font-black text-sm ${hasVoted ? 'text-white' : 'text-gray-500'}`}>
                    {item.votes || 0}
                  </Text>
                </TouchableOpacity>
                ...
                <View className="flex-row gap-x-2">
                  <GlassButton
                    variant="icon"
                    icon={<Check size={20} color="#10B981" strokeWidth={3} />}
                    onPress={() => handleApprove(item.id)}
                  />
                  <GlassButton
                    variant="icon"
                    icon={<X size={20} color="#EF4444" strokeWidth={3} />}
                    onPress={() => setRejectTarget(item.id)}
                  />
                </View>
              </View>
  ...
  return (
    <GlassScreen bgVariant="glass" safeAreaEdges={['top']}>
      <GlassHeader
        ...
        transparent
        rightAction={
          <GlassButton variant="icon" icon={<MessageSquarePlus size={20} color="#6366F1" />} onPress={() => inputRef.current?.focus()} />
        }
      />
      ...
      <View className="px-6 mb-2">
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mt-2" contentContainerStyle={{ paddingHorizontal: 24 }}>
          <View className="flex-row items-center gap-x-2 pb-1">
            {dates.map((dateOption) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  className={`px-5 py-2.5 rounded-[18px] border outline-none shadow-sm ${isSelected ? 'bg-indigo-600/90 border-indigo-500 shadow-indigo-300' : 'bg-white/40 border-white/60'
                    }`}
                >
                  <Text className={`text-[10px] font-black uppercase tracking-widest ${isSelected ? 'text-white' : 'text-gray-500'}`}>
                    {dateOption.label}
                  </Text>
                </TouchableOpacity>
              );
      ...
      <Modal visible={showApproveModal} transparent animationType="slide" onRequestClose={() => setShowApproveModal(false)}>
        <View className="flex-1 justify-end bg-black/40 p-4 pb-10">
          <View className="bg-white/95 rounded-[40px] p-8 shadow-2xl">
            <View className="flex-row justify-between items-center mb-8">
              <View>
                <Text className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Godkjenn forslag</Text>
                <Text className="text-2xl font-black text-gray-900 uppercase tracking-tight">Velg dato</Text>
              </View>
              <GlassButton variant="icon" icon={<X size={20} color="#6B7280" />} onPress={() => setShowApproveModal(false)} />
            </View>
            <View className="gap-y-2 mb-8">
              {dates.map((d) => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                  className={`p-5 rounded-[24px] border outline-none shadow-sm ${approveDate === d.value ? 'bg-indigo-600 border-indigo-500 shadow-indigo-300' : 'bg-white/40 border-white/60'}`}
                >
                  <Text className={`font-black uppercase tracking-widest text-xs ${approveDate === d.value ? 'text-white' : 'text-gray-500'}`}>
                    {d.label}
                  </Text>
                </TouchableOpacity>
              ))}
      ...
      {rejectTarget && (
        <Modal visible transparent animationType="fade">
          <View className="flex-1 justify-center items-center bg-black/40 p-6">
            <View className="bg-white/95 rounded-[32px] p-8 w-full shadow-2xl">
              <View className="items-center mb-6">
                <View className="w-16 h-16 bg-red-50 rounded-full items-center justify-center mb-4 border border-red-100/50">
                  <Trash size={32} color="#EF4444" />
                </View>
                <Text className="text-2xl font-black text-gray-900 mb-2">Fjerne forslag?</Text>
                <Text className="text-center text-gray-500 font-medium">Er du sikker på at du vil slette dette forslaget?</Text>
              </View>
              <View className="flex-row gap-x-3">
                <GlassButton title="Avbryt" variant="secondary" onPress={() => setRejectTarget(null)} className="flex-1" />
                <GlassButton title="Fjern forslag" variant="danger" onPress={handleReject} className="flex-1" />
              </View>
            </View>
          </View>
        </Modal>
