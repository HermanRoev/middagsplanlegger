...
export function AddMealToPlanView({ meal, selectedDate, onBack, onPlanSaved, existingPlanId }: AddMealToPlanViewProps) {
    const [servingsToPlan, setServingsToPlan] = useState(meal.servings || 1);
    const [isLoading, setIsLoading] = useState(false);
    const [scaledIngredients, setScaledIngredients] = useState<Ingredient[]>(meal.ingredients || []);
    ...
    return (
        <div>
            ...
            <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="md:w-2/3">
                    ...
                    <div className="flex gap-4 text-gray-600 mb-4">
                        <p className="flex items-center gap-1">Originaloppskrift for {meal.servings || '?'} porsjoner</p>
                        {(meal.prepTime ?? 0) > 0 && (
                    ...
                    <div className="mb-6">
                        ...
                        <input
                            ...
                            onChange={(e) => setServingsToPlan(parseInt(e.target.value, 10) || 0)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
                <div>
                    <h4 className="text-lg font-semibold mb-2 text-gray-800">Ingredienser (for {servingsToPlan} porsjoner)</h4>
                    <ul className="list-disc list-inside space-y-1 text-gray-700">
                        {scaledIngredients?.map((ing, index) => (
                            <li key={index}>
                                <span className="font-medium">{formatAmount(ing.amount)} {ing.unit}</span> {ing.name}
                            </li>
                        ))}
                    </ul>
                </div>
                <div>
                    ...
                    <div className="text-gray-700 whitespace-pre-wrap">{meal.instructions || 'Instruksjoner mangler.'}</div>
                </div>
            </div>
