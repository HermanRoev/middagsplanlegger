...
export function CalendarView() {
    ...
    const [plannedMeals, setPlannedMeals] = useState<Map<string, PlannedMeal>>(new Map());
    const [isLoading, setIsLoading] = useState(true);

    const [isModalOpen, setIsModalOpen] = useState(false);
    const [selectedDate, setSelectedDate] = useState<Date | null>(null);
    const [modalView, setModalView] = useState<'library' | 'addToPlan' | 'viewMeal'>('library');
    const [selectedMeal, setSelectedMeal] = useState<Meal | null>(null);
    const [activePlannedMeal, setActivePlannedMeal] = useState<PlannedMeal | null>(null);

    const fetchPlannedMeals = useCallback(async () => {
        setIsLoading(true);
        const start = startOfMonth(currentDate);
        const end = endOfMonth(currentDate);
        const q = query(collection(db, 'mealPlans'), where('date', '>=', format(start, 'yyyy-MM-dd')), where('date', '<=', format(end, 'yyyy-MM-dd')));
    ...
    return (
        ...
        <div className="flex items-center justify-between mb-6">
            <button onClick={() => setCurrentDate(subMonths(currentDate, 1))} className="flex items-center gap-1 px-4 py-2 bg-blue-100 rounded-lg hover:bg-blue-200 text-blue-800 font-semibold transition-colors shadow-sm">
            ...
            <button onClick={() => setCurrentDate(addMonths(currentDate, 1))} className="flex items-center gap-1 px-4 py-2 bg-blue-100 rounded-lg hover:bg-blue-200 text-blue-800 font-semibold transition-colors shadow-sm">
                Neste
                <span className="material-icons text-base align-middle">arrow_forward</span>
            </button>
        </div>

        <div className="grid grid-cols-7 gap-2 text-center font-semibold text-gray-600 pb-2 mb-2">
            {['Man', 'Tir', 'Ons', 'Tor', 'Fre', 'Lør', 'Søn'].map(day => <div key={day} className="py-2">{day}</div>)}
        </div>

        <div className="grid grid-cols-7 gap-2 flex-grow">
            {Array.from({ length: startingDayIndex }).map((_, index) => (
                <div key={`empty-${index}`} className="border border-gray-200 rounded-xl bg-gray-50 shadow-sm"></div>
            ))}
            {isLoading ? (
                Array.from({ length: daysInMonth.length }).map((_, index) => (
                    <div key={index} className="border border-gray-200 rounded-xl p-2 flex flex-col bg-white shadow-sm hover:shadow-lg transition-all duration-200 relative w-full h-[180px]">
                ...
                daysInMonth.map((day) => {
                    ...
                    return (
                        <div key={dateKey} className={`border border-gray-200 rounded-xl p-2 flex flex-col cursor-pointer bg-white shadow-sm hover:shadow-lg hover:bg-blue-50 transition-all duration-200 relative w-full h-[180px] ${isCurrentDay ? 'ring-2 ring-blue-500' : ''}`} onClick={() => handleDayClick(day)}>
                            {plannedMeal?.isShopped && (
                                <div className="absolute top-1 right-1 bg-green-500 text-white rounded-full h-5 w-5 flex items-center justify-center text-xs shadow">✓</div>
                            )}
        ...
        {selectedDate && (
            <Modal
                ...
                title={
                    modalView === 'library' ? `Velg middag for ${format(selectedDate, 'eeee d. MMMM', { locale: nb })}` :
                    modalView === 'viewMeal' ? activePlannedMeal?.mealName || 'Se planlagt middag' :
            ...
            >
                ...
                {modalView === 'addToPlan' && selectedMeal && (
                    <AddMealToPlanView 
                        ...
                        onBack={() => setModalView(activePlannedMeal ? 'viewMeal' : 'library')} 
                    ...
                    <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="md:w-2/3">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <h3 className="text-2xl font-bold text-gray-800 mb-2">{activePlannedMeal.mealName}</h3>
                                    <p className="text-gray-600">Planlagt for {activePlannedMeal.plannedServings} porsjoner</p>
                                </div>
                                <button 
                                    onClick={() => setModalView('library')}
                                    className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2"
                                >
                                    <span className="material-icons text-base align-middle">swap_horiz</span>
                                    Bytt middag
                                </button>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    <h4 className="text-lg font-semibold mb-2 text-gray-800">Ingredienser</h4>
                                ...
                                <div>
                                    <h4 className="text-lg font-semibold mb-2 text-gray-800">Instruksjoner</h4>
                                    <div className="text-gray-700 whitespace-pre-wrap">
                                        {activePlannedMeal.instructions || 'Instruksjoner mangler.'}
                                    </div>
                                </div>
                            </div>
                        </div>
