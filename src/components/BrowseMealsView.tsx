...
export function BrowseMealsView() {
    ...
    const handleDeleteMeal = async (mealId: string, mealName: string) => {
        if (window.confirm(`Er du sikker på at du vil slette '${mealName}'? Denne handlingen kan ikke angres.`)) {
    ...
    return (
        <div className="bg-white p-8 rounded-xl shadow-xl w-full">
            <h2 className="text-3xl font-bold mb-6 text-gray-800">Middagsbibliotek</h2>
            ...
            {activeMeal && (
                <Modal isOpen={!!activeMeal} onClose={handleCloseDetail} title={activeMeal.name}>
                    <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="md:w-2/3">
                            <h3 className="text-2xl font-bold text-gray-800 mb-2">{activeMeal.name}</h3>
                            <div className="flex gap-4 text-gray-600 mb-4">
                                <p>Originaloppskrift for {activeMeal.servings || '?'} porsjoner</p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-4">
                                    ...
                                    <ul className="list-disc list-inside space-y-1 text-gray-700">
                                        {activeMeal.ingredients?.map((ing, idx) => (
                                            <li key={idx}>
                                                ...
                                                ) : null} {typeof ing === 'string' ? ing : ing.name}
                                            </li>
                                        ))}
                                    </ul>
                                </div>
                                <div>
                                    ...
                                    <div className="text-gray-700 whitespace-pre-wrap">{activeMeal.instructions || 'Instruksjoner mangler.'}</div>
                                </div>
                            </div>
                            <div className="flex gap-4 mt-4">
                                <Link href={`/meals/edit/${activeMeal.id}`} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2">
                                    <span className="material-icons text-base align-middle">edit</span>
                                    Rediger
                                </Link>
                                <button className="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 flex items-center gap-2" onClick={() => { handleDeleteMeal(activeMeal.id, activeMeal.name); handleCloseDetail(); }}>
            ...
            {filteredMeals.length === 0 && !isLoading && (
                <p className="text-center text-gray-500 mt-8">Ingen middager funnet som passer søket.</p>
            )}
        </div>
    );
}