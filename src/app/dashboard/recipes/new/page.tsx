  ...
  const handleImport = async (importType: 'image' | 'text' | 'generate') => {
    ...
    try {
      ...
      if (parsedData) {
        ...
        toast.success('Oppskrift analysert! Gå gjennom detaljene.', { id: toastId })
      ...
      toast.error(`Feil: ${message}`, { id: toastId })
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <PageContainer className="space-y-8 pb-12">
      ...
      <Tabs defaultValue="text" className="w-full">
        <TabsList className="grid w-full grid-cols-4 mb-6 h-11 bg-gray-100/50 p-1 rounded-xl border border-gray-100">
          <TabsTrigger value="text" className="rounded-lg font-semibold data-[state=active]:bg-white data-[state=active]:shadow-sm text-sm">Beskriv</TabsTrigger>
          <TabsTrigger value="generate" className="rounded-lg font-semibold data-[state=active]:bg-white data-[state=active]:shadow-sm text-sm">AI Forslag</TabsTrigger>
          <TabsTrigger value="image" className="rounded-lg font-semibold data-[state=active]:bg-white data-[state=active]:shadow-sm text-sm">Fra Bilde</TabsTrigger>
          <TabsTrigger value="manual" className="rounded-lg font-semibold data-[state=active]:bg-white data-[state=active]:shadow-sm text-sm">Manuelt</TabsTrigger>
        </TabsList>
